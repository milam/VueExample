...
const postcssPresetEnv = require('postcss-preset-env');// We are getting 'process.env.NODE_ENV' from the NPM scripts
...
const devMode = process.env.NODE_ENV !== 'production'; module.exports = {
    ...
    mode: devMode ? 'development' : 'production',// Webpack needs to know where to start the bundling process,
    // so we define the Sass file under './Styles' directory
    // and the script file under './Scripts' directory
    entry: {
        css: path.resolve(__dirname, './Styles/site.scss'),
        site: path.resolve(__dirname, './Scripts/site.js')
    },// This is where we define the path where Webpack will place
    // a bundled JS file.
    output: {
        path: path.resolve(__dirname, './wwwroot'),// Specify the base path for all the styles within your
        // application. This is relative to the output path, so in
        // our case it will be ./wwwroot/css
        publicPath: '/css',// The name of the output bundle. Path is also relative
        ...
        rules: [
            {
                ...
                test: /\.(js)$/,// Ignore the node_modules directory
                exclude: /node_modules/,// Use Babel to transpile ES6+ to ES5
                use: {
                    loader: 'babel-loader',
                    options: {
            ...
            },
            {
                ...
                // rules specified in the different loaders
                test: /\.(sa|sc)ss$/,// Use the following loaders from right-to-left, so it will
                ...
                use: [
                    ...
                    },
                    {
                        ...
                        options: {
                            ident: 'postcss',// We instruct PostCSS to autoprefix and minimize our 
                ...
                use: [
                    {
                        ...
                        options: {
                            ...
                            // extension
                            name: '[name].[ext]',// Indicates where the images are stored and will use
                            ...
                            // to '/css/site.css'
                            publicPath: '../images',// When this option is 'true', the loader will emit the
